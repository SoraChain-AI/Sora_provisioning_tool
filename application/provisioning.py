...
Integrates with the NVFlare provisioner API using the same approach as blob.py
"""
...
from .models import Project, Server, Client, Admin, User, Organization, Role
...
def _get_provisioner(root_dir: str, scheme, docker_image=None):
    """Get NVFlare provisioner instance - same as in blob.py"""
    overseer_agent = {
...
class NVFlareProvisioningService:
    """Service for generating NVFlare project configurations using provisioner API"""
    ...
    def call_nvflare_provision(self, project_id, custom_workspace=None, force_reprovision=False):
        """Call the NVFlare provisioner API directly - same approach as blob.py"""
        if not NVFLARE_AVAILABLE:
            # Fallback to CLI approach
            return self._call_nvflare_provision_cli(project_id, custom_workspace, force_reprovision)
        ...
        # Use the same approach as blob.py
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            # Create provisioner project - same as in blob.py
            prov_project = ProvProject(
                project.short_name or project.name,
                project.description,
                props=proj_props,
                root_private_key=None,  # Will be generated if not exists
                serialized_root_cert=None,  # Will be generated if not exists
            )
            
            # Add server - same as in blob.py
            primary_server = servers[0]
            server_props = {
                PropKey.FED_LEARN_PORT: primary_server.fed_learn_port,
                PropKey.ADMIN_PORT: primary_server.admin_port,
                PropKey.DEFAULT_HOST: primary_server.name,            
                PropKey.CONN_SECURITY: primary_server.connection_security or "mtls",
            }
            if project.server_props:
                server_props.update(json.loads(project.server_props))
            
            # Get organization from server
            org = primary_server.org or "nvflare"
            ...
            shutil.copytree(result_dir, workspace)
            print(f"Provisioning successful. Workspace: {workspace}")
            
            return workspace
    
    def _call_nvflare_provision_cli(self, project_id, custom_workspace=None, force_reprovision=False):
        """Fallback to CLI approach if NVFlare is not available"""
        project = Project.query.get(project_id)
        if not project:
            raise ValueError(f"Project {project_id} not found")
